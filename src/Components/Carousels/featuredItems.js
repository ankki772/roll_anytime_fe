...
export default function FeaturedItems({ populatItem }) {
  const navigate = useNavigate();

  const navigateProDetail = (val)=>{
    ...
    <div className="featured_container">
      <div className="featured_content">
        ...
        <div className="f_cards">
          {populatItem && populatItem?.length && populatItem?.map((item) => (
            // <Link>

            
              <div className="featured_card" onClick={()=>navigateProDetail(item)}>
                <div>
                  <img src={item?.product_imges[0]} alt={item?.product_description} className="featured_image" loading="lazy" />
                </div>
